...
window.__artifexMazeRuntimeControls = {
  buildGeneratedMaze,
  clearToBlankShape,
  plotSolution,
  repaintAll,
  findPath: (start, exit) => findPath(start, exit)
};
...
function bindRuntimeButtons() {
  ...
  captureClick('btn-apply-difficulty', () => analyseDifficultyFallback());
...
function bindWalkTestInput() {
  ...
  $('virtual-dpad')?.querySelectorAll('button[data-dpad]').forEach((button) => {
    ...
    const down = (event) => { event.preventDefault(); runtimeControls.keys[key] = true; updateDpad(); };
    const up = (event) => { event.preventDefault(); runtimeControls.keys[key] = false; updateDpad(); };
  ...
  if (!runtimeControls.walkTimer) runtimeControls.walkTimer = setInterval(moveWalkPlayer, 33);
}
...
function handleWalkKey(event, pressed) {
  ...
  event.preventDefault();
  event.stopImmediatePropagation();
  runtimeControls.keys[mapped] = pressed;
  updateDpad();
}

function updateDpad() {
  $('virtual-dpad')?.querySelectorAll('button[data-dpad]').forEach((button) => {
    button.classList.toggle('is-pressed', !!runtimeControls.keys[button.dataset.dpad]);
  });
}
...
function moveWalkPlayer() {
  ...
  let dx = 0;
  let dy = 0;
  if (runtimeControls.keys.w) dy -= 0.1;
  if (runtimeControls.keys.s) dy += 0.1;
  if (runtimeControls.keys.a) dx -= 0.1;
  if (runtimeControls.keys.d) dx += 0.1;
  if (!dx && !dy) return;
  const nx = s.player.x + dx;
  const ny = s.player.y + dy;
  if (isOpenCell(Math.floor(nx), Math.floor(ny))) {
    s.player.x = nx;
    s.player.y = ny;
    repaintPreviewOnly();
  }
...
function bindWorkspacePan() {
  ...
  target.addEventListener('mousedown', (event) => {
    ...
    runtimeControls.dragging = true;
    runtimeControls.lastX = event.clientX;
    runtimeControls.lastY = event.clientY;
    document.body.style.userSelect = 'none';
  }, true);
  window.addEventListener('contextmenu', (event) => { if (runtimeControls.dragging) event.preventDefault(); }, true);
  window.addEventListener('mousemove', (event) => {
    if (!runtimeControls.dragging) return;
    runtimeControls.panX += event.clientX - runtimeControls.lastX;
    runtimeControls.panY += event.clientY - runtimeControls.lastY;
    runtimeControls.lastX = event.clientX;
    runtimeControls.lastY = event.clientY;
    repaintPreviewOnly();
  }, true);
  window.addEventListener('mouseup', () => {
    runtimeControls.dragging = false;
    document.body.style.userSelect = '';
  }, true);
  $('btn-zoom-reset')?.addEventListener('click', () => {
    runtimeControls.panX = 0;
    runtimeControls.panY = 0;
...
function clearToBlankShape(reason = 'blank') {
  ...
  for (let y = 0; y < s.gridSize; y++) {
    for (let x = 0; x < s.gridSize; x++) {
      if (isInteriorShapeCell(x, y)) s.matrix[y][x] = 0;
    }
  }
  applySolidBorderWithOpenings();
  openEndpoint(s.start);
...
function analyseDifficultyFallback() {
  const s = state();
  if (!s) return;
  const route = findPath(s.start, s.exit);
...
function countBranchCells() {
  ...
  for (let y = 0; y < s.gridSize; y++) {
    for (let x = 0; x < s.gridSize; x++) {
      if (isOpenCell(x, y) && countOpenNeighbours(x, y) >= 3) count++;
    }
  }
  return count;
}
...
function countDeadEnds() {
  ...
  for (let y = 0; y < s.gridSize; y++) {
    for (let x = 0; x < s.gridSize; x++) {
      if (isOpenCell(x, y) && countOpenNeighbours(x, y) === 1) count++;
    }
  }
...
function repaintPreviewOnly() {
  state()?.view === '3d' ? draw3DPlaceholder() : drawPreview();
}
...
function drawPreview() {
  ...
  ctx.save();
  ctx.translate(dims.ox + runtimeControls.panX, dims.oy + runtimeControls.panY);
...
function dimensions(width, height, zoom = 1) {
  ...
  return {
    cellW,
    cellH,
    ox: width / 2 - (s.gridSize * cellW) / 2,
    oy: height / 2 - (s.gridSize * cellH) / 2
  };
}
...
function drawPlayer(ctx, cellW, cellH) {
  drawNode(ctx, { x: Math.floor(state().player.x), y: Math.floor(state().player.y) }, cellW, cellH, '#f3dcaa');
}
...
function fillBackground(ctx, w, h) {
  ctx.fillStyle = state()?.style?.roof?.color || '#031009';
...
function setStatus(text) {
  setText('player-status-indicator', text);
  updateOverviewSummary();
}
