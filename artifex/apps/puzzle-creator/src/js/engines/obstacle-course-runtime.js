// Obstacle Course / Horse Forest Runner V25
// Blur/feather is now restricted to ground surfaces only: ground outer edges + path colour transitions.
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .obstacle-three-wrap:after{content:'';position:absolute;left:0;right:0;bottom:0;height:74px;background:linear-gradient(180deg,rgba(0,0,0,0),rgba(10,8,5,.34));pointer-events:none;z-index:2}
...
function makeAlphaGradientTexture(direction = 'left') {
  const size = 128;
  const canvas = document.createElement('canvas');
  canvas.width = size;
  canvas.height = size;
  const ctx = canvas.getContext('2d');
  let grad;
  if (direction === 'left') grad = ctx.createLinearGradient(0, 0, size, 0);
  else if (direction === 'right') grad = ctx.createLinearGradient(size, 0, 0, 0);
  else grad = ctx.createLinearGradient(0, 0, 0, size);
...
async function buildGroundMaterial() {
  try {
    ...
    OC.groundEdgeMaterial = new THREE.MeshBasicMaterial({ color: 0x182011, transparent: true, opacity: 0.42, alphaMap: makeAlphaGradientTexture('left'), depthWrite: false, side: THREE.DoubleSide });
    OC.groundEndMaterial = new THREE.MeshBasicMaterial({ color: 0x182011, transparent: true, opacity: 0.34, alphaMap: makeAlphaGradientTexture('up'), depthWrite: false, side: THREE.DoubleSide });
    updateGroundRelief();
...
function buildWorld() { buildGroundChunks(); buildGroundEdgeFeathers(); buildVisiblePath(); buildTreeCorridor(); scatterForestFloorDetail(); }
...
function buildGroundEdgeFeathers() {
  if (!OC.groundEdgeMaterial) return;
  for (let d = 0; d < OC.courseLength + 300; d += SEGMENT) {
    const center = pathCenterAt(d + SEGMENT / 2);
    [-1, 1].forEach((side) => {
      const mat = OC.groundEdgeMaterial.clone();
      mat.alphaMap = makeAlphaGradientTexture(side < 0 ? 'left' : 'right');
      const feather = new THREE.Mesh(new THREE.PlaneGeometry(8.2, SEGMENT + 1.4, 4, 4), mat);
      feather.rotation.x = -Math.PI / 2;
      feather.position.set(center + side * 19.0, GROUND_Y + 0.052, -d - SEGMENT / 2);
      feather.renderOrder = 8;
      OC.world.add(feather);
    });
  }
  for (let d of [0, OC.courseLength + 260]) {
    const center = pathCenterAt(d);
    const mat = OC.groundEndMaterial.clone();
    const end = new THREE.Mesh(new THREE.PlaneGeometry(46, 13, 4, 4), mat);
    end.rotation.x = -Math.PI / 2;
    end.position.set(center, GROUND_Y + 0.054, -d);
    end.renderOrder = 9;
    OC.world.add(end);
  }
}
