// Obstacle Course / Horse Forest Runner V26
// Adds layer controls, top-down course overview, manual placement, and alpha/texture paint decals.
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .obstacle-workspace{display:grid;grid-template-columns:minmax(620px,1fr) 332px;gap:14px;align-items:start}
  ...
  OC.stage.innerHTML = `
    <div class="obstacle-workspace">
      <section class="obstacle-view-card">
        <div class="obstacle-header-line"><div><p class="eyebrow">Obstacle Course · Horse Forest Runner V26</p><h2 id="obstacle-title">Horse Forest Ride</h2><p id="obstacle-objective">Layer editor + overview/paint controls are active.</p></div><span id="obstacle-status" class="obstacle-status-pill">Ready</span></div>
        ...
        <div class="obstacle-help-strip"><span>Use overview to place/paint. Use layer controls to move, scale, hide and alpha each layer.</span><span id="hf-tree-status">GLB trees: loading…</span></div>
...
function bindControls() {
  ...
  oc$('hf-layer-visible').addEventListener('click', () => { const l = OC.layers.get(OC.selectedLayerId); if (!l) return; l.visible = !l.visible; applyLayer(l); drawFrame(); drawOverview(); });
  ...
  document.querySelector('.left-icon-bar')?.addEventListener('click', (event) => {
    if (!OC.active) return; const button = event.target.closest('.panel-nav-button'); if (!button) return; event.preventDefault(); event.stopImmediatePropagation(); showObstaclePanel(button.dataset.panel);
  }, true);
}
...
function createLayerSliders() {
  ...
  host.innerHTML = '';
  [
    ['x', 'X', -30, 30, 0.1], ['z', 'Z', -80, 80, 0.5], ['y', 'Y', -10, 10, 0.1], ['scale', 'Scale', 0.1, 4, 0.05], ['opacity', 'Alpha', 0, 1, 0.05], ['order', 'Order', -20, 40, 1],
  ].forEach(([prop, label, min, max, step]) => {
...
function populateLayerSelect() {
  ...
  select.innerHTML = '';
  [...OC.layers.values()].sort((a, b) => a.order - b.order).forEach((l) => { const opt = document.createElement('option'); opt.value = l.id; opt.textContent = `${l.label}${l.visible ? '' : ' (hidden)'}`; select.appendChild(opt); });
...
function syncLayerOutputs() {
  const l = OC.layers.get(OC.selectedLayerId); if (!l) return;
  ['x', 'z', 'y', 'scale', 'opacity', 'order'].forEach((prop) => { const out = oc$(`hf-layer-${prop}-out`); if (out) out.textContent = prop === 'order' ? String(l[prop]) : Number(l[prop]).toFixed(2); });
}

function syncLayerControls() {
  const l = OC.layers.get(OC.selectedLayerId); if (!l) return;
  ['x', 'z', 'y', 'scale', 'opacity', 'order'].forEach((prop) => { const input = oc$(`hf-layer-${prop}`); if (input) input.value = l[prop]; });
  syncLayerOutputs();
}
...
function loadTexture(url, options = {}) {
  ...
  if (options.repeat) { texture.wrapS = THREE.RepeatWrapping; texture.wrapT = THREE.RepeatWrapping; texture.repeat.set(options.repeat[0], options.repeat[1]); }
  OC.textureCache.set(key, texture); return texture;
}

function resizeRenderer() {
  if (!OC.renderer || !OC.host || !OC.camera) return;
  const width = Math.max(1, OC.host.clientWidth), height = Math.max(300, Math.round(width * 9 / 16));
  OC.camera.aspect = width / height; OC.camera.updateProjectionMatrix(); OC.renderer.setSize(width, height);
}
...
function makeAlphaTexture(kind = 'soft') {
  const size = 128, canvas = document.createElement('canvas'); canvas.width = canvas.height = size; const ctx = canvas.getContext('2d'); const g = ctx.createRadialGradient(size / 2, size / 2, 0, size / 2, size / 2, size / 2); g.addColorStop(0, 'rgba(255,255,255,.75)'); g.addColorStop(.55, 'rgba(255,255,255,.26)'); g.addColorStop(1, 'rgba(255,255,255,0)'); ctx.fillStyle = g; ctx.fillRect(0, 0, size, size); const t = new THREE.CanvasTexture(canvas); t.needsUpdate = true; return t;
}
...
async function buildGroundMaterial() {
  try {
    ...
    for (let y = 0; y < size; y += 1) for (let x = 0; x < size; x += 1) {
      const u = x / size, v = y / size; const n1 = fbm(u * 7, v * 7, 1), n2 = fbm(u * 18, v * 18, 5); const rootPatch = smoothstep(0.66, 0.94, fbm(u * 4.4 + 12, v * 4.4 - 3, 21)) * 0.22; const sideMix = smoothstep(0.18, 0.82, fbm(u * 5.5 + 31, v * 5.5 - 8, 9)); const sx = (u * size * 2.8 + n2 * 18) % size, sy = (v * size * 2.8 + n1 * 18) % size; const pathCol = sample(pathData, size, sx, sy), grassA = sample(grassAData, size, sx * 1.2, sy * 1.2), grassB = sample(grassBData, size, sx * 1.1 + 80, sy * 1.1 + 40); const sideCol = grassA.map((c, i) => c * (1 - sideMix) + grassB[i] * sideMix); const idx = (y * size + x) * 4; [0, 1, 2].forEach((i) => { colorImage.data[idx + i] = clamp(pathCol[i] * 0.92 * rootPatch + sideCol[i] * (0.84 + n1 * 0.18) * (1 - rootPatch), 0, 255); }); colorImage.data[idx + 3] = 255; const bump = clamp(70 + n1 * 115 + n2 * 70 + rootPatch * 24, 0, 255); bumpImage.data[idx] = bumpImage.data[idx + 1] = bumpImage.data[idx + 2] = bump; bumpImage.data[idx + 3] = 255;
    }
...
async function loadTreeModels() {
  if (OC.treeLoadStarted) return; OC.treeLoadStarted = true;
  try {
    ...
    const { GLTFLoader } = await OC.gltfLoaderPromise; const loader = new GLTFLoader();
    ASSETS.trees.forEach((url) => loader.load(`${url}?v=26`, (gltf) => { if (!gltf.scene) return; gltf.scene.traverse((n) => { if (n.isMesh) n.receiveShadow = true; }); OC.treeModels.push(gltf.scene); oc$('hf-tree-status').textContent = `GLB trees: ${OC.treeModels.length} loaded`; if (OC.active) regenerateCourse(); }, undefined, (err) => console.warn('[HorseForest] GLB tree failed:', url, err)));
  } catch (error) { console.warn('[HorseForest] GLB tree loader failed.', error); oc$('hf-tree-status').textContent = 'GLB trees failed to load'; }
}

function regenerateCourse() {
  if (!OC.world) return;
  clearWorld(); OC.layers.clear();
  const template = TEMPLATES[OC.templateId] || TEMPLATES.horse_forest_easy; OC.courseLength = Math.max(900, OC.duration * OC.speed);
  OC.scene.background = loadTexture(ASSETS.background); OC.scene.fog = new THREE.Fog(template.fog, 70, 375); OC.renderer?.setClearColor(template.clear, 1);
  resetRun(true); buildWorld(); addObstacles(Math.round((8 + OC.difficulty * 5) * template.obstacleRate)); addCollectibles(8 + OC.difficulty * 3); updateTemplateText(); updateWorldPositions(); populateLayerSelect(); drawFrame(); drawOverview();
}
...
function drawOverview() { const c = oc$('hf-overview'); if (!c) return; const ctx = c.getContext('2d'); ctx.clearRect(0, 0, c.width, c.height); ctx.fillStyle = '#111914'; ctx.fillRect(0, 0, c.width, c.height); ctx.strokeStyle = '#d09a55'; ctx.lineWidth = 6; ctx.beginPath(); for (let d = 0; d < OC.courseLength; d += 18) { const p = worldToOverview(pathCenterAt(d), -d); if (d === 0) ctx.moveTo(p.x, p.y); else ctx.lineTo(p.x, p.y); } ctx.stroke(); ctx.lineWidth = 1.5; OC.placed.forEach((o) => { const p = worldToOverview(o.x, o.z); ctx.fillStyle = o.type === 'tree' ? '#48a24a' : o.type === 'rock' ? '#888' : o.type === 'fern' ? '#64c04d' : o.type === 'collectible' ? '#eec45a' : '#b04b35'; ctx.beginPath(); ctx.arc(p.x, p.y, o.type === 'tree' ? 4 : 3, 0, Math.PI * 2); ctx.fill(); }); ctx.strokeStyle = '#eec45a'; ctx.strokeRect(1, 1, c.width - 2, c.height - 2); }
...
function showObstaclePanel(panelId) { OC.panels.querySelectorAll('[data-obstacle-panel]').forEach((p) => { p.hidden = p.dataset.obstaclePanel !== panelId; p.classList.toggle('is-active', p.dataset.obstaclePanel === panelId); }); document.querySelectorAll('.panel-nav-button').forEach((b) => b.classList.toggle('is-active', b.dataset.panel === panelId)); }
