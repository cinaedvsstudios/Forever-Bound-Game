...
function playClip(key, volume = 0.75) {
  ...
  try {
    ...
    audio.volume = Math.max(0, Math.min(1, volume));
    const play = audio.play();
    if (play && typeof play.catch === 'function') play.catch(() => blip());
    return true;
  } catch (error) {
    blip();
    return false;
...
export function playJumpSound() { playClip('snort', 0.3) || blip({ freq: 260, duration: 0.07, gain: 0.05, type: 'triangle' }); }
export function playLandSound() { playClip('land', 0.65) || blip({ freq: 90, duration: 0.08, gain: 0.07, type: 'sawtooth' }); }
export function playCollectSound() { playClip('neigh', 0.38) || blip({ freq: 640, duration: 0.08, gain: 0.05, type: 'sine' }); }
export function playHitSound() { playClip('bush', 0.55) || blip({ freq: 70, duration: 0.13, gain: 0.08, type: 'square' }); }
