...
import { ASSETS, TEMPLATES, GLB_ASSETS } from './obstacle-course-assets.js?v=3.0.42';
...
function pathEdgeX(rng, distance, side) { return pathCenterAt(distance) + side * (pathHalfWidthAt(distance) + randFrom(rng, 0.55, 1.25)); }
function pathEdgeBushX(rng, distance, side) { return pathCenterAt(distance) + side * Math.max(0.1, pathHalfWidthAt(distance) - randFrom(rng, 0.02, 0.16)); }
function pathEdgeGrassX(rng, distance, side) { return pathCenterAt(distance) + side * (pathHalfWidthAt(distance) + randFrom(rng, 0.02, 0.68)); }
function limitedOutsideX(rng, distance, side, minFromEdge = 0.85, maxFromEdge = TREE_OUTER_LIMIT_FROM_PATH_EDGE) { return pathCenterAt(distance) + side * (pathHalfWidthAt(distance) + randFrom(rng, minFromEdge, maxFromEdge)); }
function limitedDetailX(rng, distance, side, minFromEdge = 0.08, maxFromEdge = DETAIL_OUTER_LIMIT_FROM_PATH_EDGE) { return pathCenterAt(distance) + side * (pathHalfWidthAt(distance) + randFrom(rng, minFromEdge, maxFromEdge)); }
...
export function scatterScenery() {
  ...
  if (tallBushAssets.length) makeDistances(rng, DENSITY_PER_1000.tallPathBushPairs * sections * template.detailRate, 22, OC.courseLength + 60, 7).forEach((d) => [-1, 1].forEach((side) => queuePlacement(rng, queues, detailLayer, null, 'detail', tallBushAssets, fallbackDetail, pathEdgeBushX(rng, d, side), 0, -d + randFrom(rng, -3, 3), randFrom(rng, 4.22, 6.02))));
  makeDistances(rng, DENSITY_PER_1000.pathEdgeGrassPairs * sections * template.detailRate, 10, OC.courseLength + 70, 4).forEach((d) => [-1, 1].forEach((side) => queuePlacement(rng, queues, detailLayer, null, 'detail', groundGrassAssets, fallbackDetail, pathEdgeGrassX(rng, d, side), -0.32, -d + randFrom(rng, -1.4, 1.4), randFrom(rng, 0.68, 1.24))));
  makeDistances(rng, DENSITY_PER_1000.groundGrassPairs * sections * template.detailRate, 12, OC.courseLength + 70, 5).forEach((d) => [-1, 1].forEach((side) => queuePlacement(rng, queues, detailLayer, null, 'detail', groundGrassAssets, fallbackDetail, limitedDetailX(rng, d, side, 0.04, 2.1), -0.3, -d + randFrom(rng, -2.2, 2.2), randFrom(rng, 0.5, 1.05))));
