...
function fallbackRock(rng = Math.random) { const rock = new THREE.Mesh(new THREE.DodecahedronGeometry(randFrom(rng, .64, 1.5), 0), new THREE.MeshStandardMaterial({ color: 0x7d776b })); rock.position.y = GROUND_Y + 0.76; return rock; }
function fallbackDetail(rng = Math.random) { const detail = new THREE.Mesh(new THREE.ConeGeometry(randFrom(rng, .36, .7), randFrom(rng, .8, 1.5), 5), new THREE.MeshStandardMaterial({ color: 0x2e8b45 })); detail.position.y = GROUND_Y + 0.7; return detail; }

function scatterX(rng, distance, side, fromHalf, toHalf) { const center = pathCenterAt(distance); const half = pathHalfWidthAt(distance); return center + side * (half + randFrom(rng, fromHalf, toHalf)); }
...
function queuePlacement(rng, queues, layer, type, assetList, fallbackFactory, x, groundOffset, z, scale = 1) {
  ...
  queues.get(key).placements.push({ x, y: GROUND_Y + groundOffset, z, rotationY: randFrom(rng, 0, Math.PI * 2), scale: scale * (asset.scale || 1) });
...
export function scatterScenery() {
  ...
  const rng = seededRandom(`${OC.templateId}|${OC.difficulty}|${OC.courseLength}|${OC.groundPathMap?.sequence?.join(',') || '1'}`);
  ...
  makeDistances(rng, DENSITY_PER_1000.rocks * sections * template.rockRate, 38, OC.courseLength + 40, 18).forEach((d) => {
    ...
    queuePlacement(rng, queues, rockLayer, 'rock', rockAssets, fallbackRock, scatterX(rng, d, side, .8, 8), 0, -d + randFrom(rng, -4, 4), randFrom(rng, .7, 1.0));
  });
  makeDistances(rng, DENSITY_PER_1000.edgeDetailPairs * sections * template.detailRate, 20, OC.courseLength + 40, 14).forEach((d) => {
    [-1, 1].forEach((side) => queuePlacement(rng, queues, detailLayer, 'detail', edgeDetailAssets, fallbackDetail, scatterX(rng, d, side, .2, 3.0), 0, -d + randFrom(rng, -3, 3), randFrom(rng, .75, 1.15)));
  });
  makeDistances(rng, DENSITY_PER_1000.farDetailPairs * sections * template.detailRate, 50, OC.courseLength + 60, 22).forEach((d) => {
    [-1, 1].forEach((side) => queuePlacement(rng, queues, detailLayer, 'detail', farDetailAssets, fallbackDetail, scatterX(rng, d, side, 5, 13), 0, -d + randFrom(rng, -5, 5), randFrom(rng, .65, 1.0)));
  });
