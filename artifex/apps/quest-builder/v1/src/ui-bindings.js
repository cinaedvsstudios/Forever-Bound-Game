import { THUMBNAILS } from './module-config.js?v=1.2.12';
import { BLOCK_TYPES } from './block-types.js?v=1.2.12';
import { parseList, escapeHtml } from './quest-schema.js?v=1.2.12';
import { openEditor, createWizardQuest, wireEditorTabs } from './dialog-editors.js?v=1.2.12';
import { exportQuestFile, buildQuestExportBundle, downloadProjectPackageFiles, downloadJson, slugify } from './export-json.js?v=1.2.12';
import { clamp } from './layout-state.js?v=1.2.12';
...
export function closeMenus() { document.querySelectorAll('.menu-panel.open').forEach((panel) => panel.classList.remove('open')); }
...
export function wirePanelResize(app) {
  const grip = app.$('panel-resizer'); let dragging = false; let startX = 0; let startW = 0;
  grip.addEventListener('pointerdown', (event) => { dragging = true; startX = event.clientX; startW = app.layout().leftW; document.body.style.cursor = 'col-resize'; grip.setPointerCapture(event.pointerId); });
  grip.addEventListener('pointermove', (event) => { if (!dragging) return; app.patchLayout({ leftW: clamp(startW + event.clientX - startX, 230, 620) }); app.applyLayout(); });
  grip.addEventListener('pointerup', (event) => { if (!dragging) return; dragging = false; document.body.style.cursor = ''; app.saveLayout(); try { grip.releasePointerCapture(event.pointerId); } catch { /* noop */ } });
}
...
export function wireFlowDrag(app) {
  const win = app.$('flow-window'); const handle = app.$('flow-window-header');
  let dragging = false; let startX = 0; let startY = 0; let baseX = 0; let baseY = 0;
  handle.addEventListener('pointerdown', (event) => {
    if (event.target.closest('button')) return;
    event.preventDefault(); dragging = true; startX = event.clientX; startY = event.clientY; baseX = app.layout().flowX; baseY = app.layout().flowY;
    win.classList.add('dragging'); document.body.style.cursor = 'grabbing';
  });
  window.addEventListener('pointermove', (event) => {
    if (!dragging) return;
    event.preventDefault();
    app.patchLayout({ flowX: clamp(baseX + event.clientX - startX, -win.offsetWidth + 70, window.innerWidth - 70), flowY: clamp(baseY + event.clientY - startY, 0, window.innerHeight - 42) });
    app.applyLayout();
  }, { passive: false });
  window.addEventListener('pointerup', () => { if (!dragging) return; dragging = false; win.classList.remove('dragging'); document.body.style.cursor = ''; app.saveLayout(); });
  window.addEventListener('pointercancel', () => { dragging = false; win.classList.remove('dragging'); document.body.style.cursor = ''; });
}

export function wireWorkspacePan(app) {
  const workspace = app.$('workspace'); let panning = false; let startX = 0; let startY = 0; let baseX = 0; let baseY = 0;
  workspace.addEventListener('mousedown', (event) => {
    if (!(event.button === 1 || (app.layout().panMode && event.button === 0))) return;
    event.preventDefault(); event.stopPropagation(); panning = true; startX = event.clientX; startY = event.clientY; baseX = app.layout().panX; baseY = app.layout().panY; workspace.classList.add('panning'); document.body.style.cursor = 'grabbing';
  }, true);
  workspace.addEventListener('auxclick', (event) => { if (event.button === 1) { event.preventDefault(); event.stopPropagation(); } }, true);
  window.addEventListener('mousemove', (event) => { if (!panning) return; event.preventDefault(); app.patchLayout({ panX: baseX + event.clientX - startX, panY: baseY + event.clientY - startY }); app.applyCanvasTransform(); }, { passive: false });
  window.addEventListener('mouseup', () => { if (!panning) return; panning = false; workspace.classList.remove('panning'); document.body.style.cursor = ''; app.saveLayout(); });
...
export function wireInputs(app) {
  ...
  bind(app, 'selected-block-type', (value) => { const block = app.block(); block.type = value; if (!block.thumbnail) block.thumbnail = app.meta(value).emoji; });
  ...
  bind(app, 'block-type-input', (value) => { const block = app.block(); block.type = value; if (!block.name || Object.values(BLOCK_TYPES).some((item) => item.name === block.name)) block.name = app.meta(value).name; if (!block.thumbnail) block.thumbnail = app.meta(value).emoji; });
...
function setPrimaryField(block, field, value) { if (block) block[field || 'action'] = value; }
function bind(app, id, fn) { app.$(id)?.addEventListener('input', (event) => { if (app.quest() || id.startsWith('file')) { fn(event.target.value); app.render(); } }); }
...
export function wireActions(app) {
  ...
  app.$('quest-thumb-button').onclick = () => { const quest = app.quest(); quest.thumbnail = THUMBNAILS[(THUMBNAILS.indexOf(quest.thumbnail || '🏆') + 1) % THUMBNAILS.length]; app.render(); };
  app.$('block-thumb-button').onclick = () => { const block = app.block(); block.thumbnail = THUMBNAILS[(THUMBNAILS.indexOf(block.thumbnail || app.meta(block.type).emoji) + 1) % THUMBNAILS.length]; app.render(); };
  app.$('lock-selected-button').onclick = () => { app.state.selectedLocked = !app.state.selectedLocked; app.$('selected-card').classList.toggle('locked', app.state.selectedLocked); app.$('lock-selected-button').textContent = app.state.selectedLocked ? '🔒' : '🔓'; };
  app.$('collapse-selected-button').onclick = () => { app.$('selected-card').classList.toggle('collapsed'); app.$('collapse-selected-button').textContent = app.$('selected-card').classList.contains('collapsed') ? '▸' : '▾'; };
  document.querySelectorAll('[data-add-block]').forEach((button) => { button.onclick = () => { app.addBlock(button.dataset.addBlock); closeMenus(); openEditor(app, 'block'); app.toast('Block added loose. Drag its link icon onto another card to connect it.'); }; });
  ...
  app.$('export-project-files-button').onclick = () => { const count = downloadProjectPackageFiles(buildQuestExportBundle(app.doc)); closeMenus(); app.toast(`Started ${count} project file downloads.`); };
  app.$('view-json-button').onclick = () => { const bundle = buildQuestExportBundle(app.doc); app.$('json-preview-summary').innerHTML = renderExportSummary(bundle); app.$('json-preview').textContent = JSON.stringify(bundle, null, 2); closeMenus(); app.$('json-dialog').showModal(); };
  app.$('block-types-button').onclick = () => { const rows = Object.entries(BLOCK_TYPES).map(([key, item]) => `<article class="block-type-card"><strong>${item.emoji} ${item.name}</strong><span>${key} / ${item.category} / primary: ${item.primaryField}</span><span>required: ${(item.requiredFields || []).join(', ') || 'none'}</span><span>${item.hint}</span></article>`).join(''); app.help('Quest Block Type List', `<div class="block-type-grid">${rows}</div>`); closeMenus(); };
  ...
  app.$('quick-start-button').onclick = () => { app.help('Quick Start', '<p>Add blocks as loose workspace cards. Move a card by dragging its body. Turn on 🧲 to snap moved cards to the smaller grid. Create flow by dragging the 🔗 icon onto another block or END; the connector will automatically use the shortest card edges. Click an attached circle to clear that line.</p>'); closeMenus(); };
  ...
  app.$('snap-grid-button').onclick = () => { const active = !app.layout().snapToGrid; app.patchLayout({ snapToGrid: active }); app.saveLayout(); app.applyLayout(); app.toast(active ? 'Snap to grid on. Moved cards will align to the grid.' : 'Snap to grid off.'); };
