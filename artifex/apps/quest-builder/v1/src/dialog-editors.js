...
export function fillEditorInputs(app) {
  ...
  const b = app.block() || {};
  app.$('editor-thumb').textContent = b.thumbnail || app.meta(b.type).emoji;
...
function wizardBlocks(flow, sceneId, objectId, flag) {
  ...
  if (flow === 'dialogue') return [start, { name: 'Speak With Character', type: 'action', thumbnail: '👉', objectId, action: 'speak:' + objectId }, { name: 'Linked Dialogue', type: 'dialogue', thumbnail: '💬', objectId, dialogueId: '' }, done];
  if (flow === 'puzzle') return [start, { name: 'Find Clue', type: 'information', thumbnail: '🔎', sceneId }, { name: 'Check Requirement', type: 'condition', thumbnail: '🔀' }, { name: 'Capra Hint', type: 'capra', thumbnail: '🐐', uiOverlay: 'capra_popup' }, done];
  if (flow === 'combat') return [start, { name: 'Defeat Foe', type: 'combat', thumbnail: '⚔️', sceneId }, { name: 'Reward', type: 'reward', thumbnail: '🎁' }, done];
  return [start, { name: 'Key Interaction', type: 'object', thumbnail: '🧩', objectId }, done];
}
