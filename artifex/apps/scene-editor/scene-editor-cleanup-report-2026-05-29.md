...
# Runtime module cleanup update — v0.31 — 2026-05-29

## Starting state for this pass

- Repository branch: `work`.
- Starting HEAD before this pass: `8d0fcec Split Scene Editor core into modular files and archive legacy helpers`.
- Prior deployed cleanup version: `v0.30-scene-core-split`.
- This pass version/cache label: `v0.31-runtime-module-cleanup`.

## Runtime module renames

The six still-loaded numbered feature modules were renamed with `git mv` to permanent ownership-based names:

| Old loaded filename | New loaded filename |
| --- | --- |
...
No active transform, slider, offscreen placement, or menu-controller filenames were changed.

## Stale runtime version/toast cleanup

- `scene-editor-asset-path-tools.js` now derives its helper toast/status prefix from `window.ArtifexSceneEditorCore.getVersion()`, then `window.ArtifexSceneEditorConfig.VERSION`, then `v0.31-runtime-module-cleanup` as a fallback.
- `scene-editor-layer-controls.js` now uses the same active-version fallback chain for helper toasts.
- The old hardcoded `v0.28-consolidation` helper label was removed from active Scene Editor runtime JavaScript.
- The existing helper messages and behaviours were preserved; only the displayed version prefix changed.

## v0.31 version/cache sync

- `scene-editor-config.js` declares `v0.31-runtime-module-cleanup`.
- `scene-editor-app.js` fallback version is `v0.31-runtime-module-cleanup`.
- `index.html` title and all active Scene Editor CSS/JS cache query values use `v0.31-runtime-module-cleanup`.
- The shared active-project client remains at its own `v=1.0.0` cache key.

## Final active load list after v0.31 cleanup

CSS:

1. `./scene-editor.css?v=v0.31-runtime-module-cleanup`
2. `./context-menu.css?v=v0.31-runtime-module-cleanup`
3. `./scene-editor-panel-stage.css?v=v0.31-runtime-module-cleanup`
4. `./scene-editor-control-cards.css?v=v0.31-runtime-module-cleanup`
5. `./scene-editor-value-sliders.css?v=v0.31-runtime-module-cleanup`
6. `./scene-editor-ui-polish.css?v=v0.31-runtime-module-cleanup`

JavaScript:

1. `../../shared/active-project/active-project-client.js?v=1.0.0`
2. `./scene-editor-config.js?v=v0.31-runtime-module-cleanup`
3. `./scene-editor-storage.js?v=v0.31-runtime-module-cleanup`
4. `./scene-editor-scene-model.js?v=v0.31-runtime-module-cleanup`
5. `./scene-editor-io.js?v=v0.31-runtime-module-cleanup`
6. `./scene-editor-stage-drag.js?v=v0.31-runtime-module-cleanup`
7. `./scene-editor-renderer.js?v=v0.31-runtime-module-cleanup`
8. `./scene-editor-bindings.js?v=v0.31-runtime-module-cleanup`
9. `./scene-editor-core-api.js?v=v0.31-runtime-module-cleanup`
10. `./scene-editor-app.js?v=v0.31-runtime-module-cleanup`
11. `./scene-editor-v2.js?v=v0.31-runtime-module-cleanup`
12. `./scene-editor-asset-path-tools.js?v=v0.31-runtime-module-cleanup`
13. `./scene-editor-transform-controls.js?v=v0.31-runtime-module-cleanup`
14. `./scene-editor-layer-controls.js?v=v0.31-runtime-module-cleanup`
15. `./scene-editor-value-sliders.js?v=v0.31-runtime-module-cleanup`
16. `./scene-editor-card-controller.js?v=v0.31-runtime-module-cleanup`
17. `./scene-editor-visual-adjustments.js?v=v0.31-runtime-module-cleanup`
18. `./scene-editor-offscreen-placement.js?v=v0.31-runtime-module-cleanup`
19. `./scene-editor-aspect-controls.js?v=v0.31-runtime-module-cleanup`
20. `./scene-editor-object-preview.js?v=v0.31-runtime-module-cleanup`
21. `./scene-editor-menu-controller.js?v=v0.31-runtime-module-cleanup`

## Tests and checks run for v0.31

- `node --check` over every active JavaScript file loaded by `index.html`, including `../../shared/active-project/active-project-client.js`.
- `npm run dev -- --host 127.0.0.1`.
- `curl` index check at `http://127.0.0.1:5173/artifex/apps/scene-editor/index.html?fresh=v0.31-runtime-module-cleanup` — HTTP 200.
- Python/curl active asset check from `index.html` — every linked active CSS/JS asset returned HTTP 200.
- `curl` template manifest check at `http://127.0.0.1:5173/artifex/templates/templates.json` — HTTP 200.
- Temporary Playwright setup in `/tmp/pwtest` with browser/dependency installation outside tracked repo files.
- Temporary Playwright v0.31 smoke script verified page render, `v0.31-runtime-module-cleanup` display, absence of visible `v0.28-consolidation`, local backup/resume, import menu, object selection, move handle, resize handle, rotate handle, origin marker, layer recalculate button, card toggle, visual opacity control, aspect lock button, object preview button/modal, context menu, JSON download event, and `window.ArtifexSceneEditorCore` compatibility.

## Visual tests still required after deployment

A headless smoke test passed, but the user should still perform visual regression after deployment for:

- Overall layout/styling at target device sizes.
- Pointer feel and exact placement while dragging move, resize, and rotate handles.
- Origin marker visual position.
- Layer controls, card controls, visual adjustments, aspect controls, object preview, context menu, and downloaded JSON flow in the target browser/OS.

## Cleanup closure status

The Scene Editor cleanup list can be closed after user visual verification of `v0.31-runtime-module-cleanup`. The later cross-app active-project/reference/index integration phase can begin separately; this pass intentionally did not change cross-app behaviour.
