...
function drawPortalInnerWisps(ctx, layer, g, t) {
  ...
  for (let i = 0; i < amount; i += 1) {
    ...
    const color = pickColor(layer, h, 'innerWispColorA', 'innerWispColorB');
    ctx.strokeStyle = withAlpha(color, opacity * (0.46 + h * 0.3));
...
function drawPortalLineOutline(ctx, layer, g, t) {
  ...
  const color = resolveModeColor(layer, layer.outlineColorMode || 'animated', t, 'outlineColorA', 'outlineColorB');
  ctx.save();
  ctx.globalCompositeOperation = 'screen';
...
function drawPortalParticles(ctx, layer, g, t) {
  ...
  const pulse = 1 + Math.sin(t * 2.7) * percent(layer.particlePulseStrength, 10) * 0.45;
  ctx.save();
  ctx.globalCompositeOperation = 'screen';
  ctx.shadowBlur = glow;
  for (let i = 0; i < amount; i += 1) {
    ...
    ctx.fillStyle = withAlpha(color, opacity * (0.25 + hash(i + 3) * 0.55) * pulse);
    ctx.shadowColor = color;
    ctx.beginPath();
    ctx.arc(g.x + Math.cos(a) * r, g.y + Math.sin(a) * r, size * (0.45 + hash(i + 4)) * pulse, 0, TAU);
    ctx.fill();
  }
  ctx.restore();
}
...
function drawWormholeArms(ctx, layer, g, t) {
  ...
  const pulse = 1 + Math.sin(t * 2.2) * percent(layer.armPulseStrength, 10) * 0.35;
  ctx.save();
  ctx.globalCompositeOperation = 'screen';
  ...
  for (let i = 0; i < amount; i += 1) {
    ...
    const len = radius * (0.45 + hash(i + 1) * 0.7) * pulse;
    const width = thickness * (0.45 + hash(i + 2) * 0.9) * pulse;
    const color = pickColor(layer, h);
    ctx.strokeStyle = withAlpha(color, clamp(opacity * (0.18 + definition * 0.22) * pulse, 0, 0.3));
...
function drawOrbitClouds(ctx, layer, g, t, mode) {
  const opacity = percent(layer.cloudOpacity, mode === 'portal' ? 24 : 14);
  const amount = Math.round(clamp(num(layer.cloudAmount, mode === 'portal' ? 24 : 18), 0, 72));
  if (opacity <= 0 || amount <= 0) return;
  const pulse = 1 + Math.sin(t * 1.8) * percent(layer.cloudPulseStrength, mode === 'portal' ? 14 : 8) * 0.28;
  const size = g.radius * ratio(layer.cloudSize, mode === 'portal' ? 14 : 12, 2, 55) * pulse;
  const radius = g.radius * ratio(layer.orbitRadius, mode === 'portal' ? 100 : 64, 8, 150) * (1 + (pulse - 1) * 0.4);
  ...
  for (let i = 0; i < amount; i += 1) {
    ...
    softBlob(ctx, g.x + Math.cos(a) * r, g.y + Math.sin(a) * r, size * (0.55 + h), pickColor(layer, h), clamp(opacity * (0.16 + h * 0.18) * pulse, 0, 0.24));
  }
  ctx.restore();
}
...
function drawWormholeParticles(ctx, layer, g, t) {
  ...
  const pulse = 1 + Math.sin(t * 2.6) * percent(layer.particlePulseStrength, 8) * 0.45;
  ctx.save();
  ctx.globalCompositeOperation = 'screen';
  ctx.shadowBlur = glow;
  for (let i = 0; i < amount; i += 1) {
    ...
    const color = pickColor(layer, h);
    ctx.fillStyle = withAlpha(color, clamp(opacity * (0.22 + h * 0.42) * pulse, 0, 0.38));
    ctx.shadowColor = color;
    ctx.beginPath();
    ctx.arc(g.x + Math.cos(a) * r, g.y + Math.sin(a) * r, size * (0.6 + hash(i + 5)) * pulse, 0, TAU);
    ctx.fill();
  }
  ctx.restore();
}
...
function makeGeometry(layer, scaleValue) {
  ...
  return { scale, x: num(layer.emitterX, 640) * scale, y: num(layer.emitterY, 360) * scale, radius: base * ratio(layer.effectScale, 62, 8, 120) * 0.42 };
}
...
function resolveModeColor(layer, mode, t, keyA, keyB) {
  if (mode === 'solid-b') return normalizeHex(layer[keyB] || layer.colorB || '#7ae8ff');
  if (mode === 'blend') return pickColor(layer, 0.5, keyA, keyB);
  if (mode === 'animated') return pickColor(layer, (Math.sin(t * 1.6) + 1) / 2, keyA, keyB);
  return normalizeHex(layer[keyA] || layer.colorA || '#eaffff');
}
function pickColor(layer, mix = 0, keyA = 'colorA', keyB = 'colorB') { return mixHex(normalizeHex(layer[keyA] || layer.colorA || '#4ff7ff'), normalizeHex(layer[keyB] || layer.colorB || '#be55ff'), clamp(mix, 0, 1)); }
...
function withAlpha(hexValue, alpha) {
