...
import { saveCurrentObjectToProject } from './object-project-storage.js?v=1.35';
...
export function createObjectWizardBuildChecklist({ wizardState, setHeader, content, renderBasicChanges }) {
  ...
  function renderRequirementDetail(req) {
    ...
    panel.innerHTML = `<h3>${emojiFor(req.actionId)} ${escapeHtml(req.label)}</h3><p class="hint">${escapeHtml(req.type)} · ${escapeHtml(req.actionId)}</p><div class="wizard-preview-stage" data-preview-stage></div><div class="wizard-preview-controls"><button type="button" data-play-toggle>${wizardState.previewPlaying ? 'Pause' : 'Play'}</button><button type="button" data-prev-frame>‹ Frame</button><span class="wizard-frame-readout" data-frame-readout>${frames.length ? currentFrame + 1 : 0}/${frames.length}</span><button type="button" data-next-frame>Frame ›</button></div><div class="wizard-build-fields"><label>Asset style<select data-build="mode"><option value="sprite_sheet">Sprite sheet</option><option value="individual_images">Individual images</option><option value="metadata">Metadata / no image</option></select></label><label>Sprite sheet / primary asset ID<input data-build="spriteSheetAssetId" placeholder="asset_character_action" /></label><label>Frame count<input data-build="frameCount" type="number" min="0" /></label><label>FPS<input data-build="fps" type="number" min="0" placeholder="8" /></label><label>Primary Sound Asset ID<input data-build="soundAssetId" placeholder="asset_sfx_object_action" title="Registered sound asset ID from assets/asset-index.json" /></label><label class="wizard-check-row" title="Marks only the selected task as ready; it does not finish or save the object."><span>Mark Task Ready</span><input data-build="complete" type="checkbox" /></label></div><label>Notes<textarea data-build="notes" rows="2" placeholder="alignment, frame order, special rules"></textarea></label><section class="wizard-correction-grid"><label>Scale correction %<input data-correct="scale" type="range" min="-10" max="10" value="${Number(data.correction?.scale || 0)}" /></label><label>Move X px<input data-correct="x" type="range" min="-10" max="10" value="${Number(data.correction?.x || 0)}" /></label><label>Move Y px<input data-correct="y" type="range" min="-10" max="10" value="${Number(data.correction?.y || 0)}" /></label><button type="button" data-match-brightness>Match brightness across frames</button></section><div class="wizard-build-actions"><label class="button-like">Add image files<input type="file" accept="image/*" multiple hidden data-frame-files /></label><button type="button" data-empty-frame>Add empty image slot</button></div><div class="wizard-frame-strip" data-frame-strip><p class="hint">Drop image files here or use Add image files. Drag boxes sideways to change order.</p></div>`;
  ...
  async function finishAndSaveToProject() {
    frameTasks.stopPreview(false);
    validateCurrentArchetype();
    const saved = await saveCurrentObjectToProject({ allowConnect: true });
    if (!saved) return;
  ...
  function finishWithoutSaving() {
    ...
    deleteWizardSession(wizardState.sessionId);
    document.getElementById('quickstart-dialog')?.close();
    window.dispatchEvent(new CustomEvent('artifex:toast', { detail: { message: 'Setup closed without writing to the project folder.', type: 'success' } }));
  }

  function saveAndResumeLater() {
    saveWizardSession();
    document.getElementById('quickstart-dialog')?.close();
    window.dispatchEvent(new CustomEvent('artifex:toast', { detail: { message: 'Wizard draft saved. Resume it from the 🔮 beside File.', type: 'success' } }));
  }
