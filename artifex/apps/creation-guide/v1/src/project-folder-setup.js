...
function renderProjectFolderSetupSection(force = false) {
  ...
  const visibleState = connected ? (projectFolderStructureWrittenThisSession ? 'Saved to Project Folder' : 'Folder Connected') : folderState.saveStatus || 'No Folder Connected';
...
async function initialiseRealProjectStructure() {
  ...
  try {
    ...
    renderProjectFolderSetupSection(true);
  } catch (error) { showProjectFolderToast(error.message || 'Could not initialise the project structure.', 'warn'); }
}
