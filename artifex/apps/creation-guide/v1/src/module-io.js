...
export function exportJson() {
  ...
  const safeName = sanitizeFilename(moduleState.document.setup?.gameTitle || moduleState.document.name || MODULE_SLUG);
  link.href = url;
  link.download = `${safeName}-creation-guide.json`;
...
export function saveLocal() {
  ...
  saves[id] = {
    ...
    name: moduleState.document.setup?.gameTitle || moduleState.document.name,
...
export function formatLocalSavesForDisplay() {
  const entries = Object.entries(readLocalSaves());
  if (!entries.length) return 'No local Creation Guide saves found.';
  return entries
