(() => {
  ...
  const root = document.querySelector('#fb-game-root');
  const manifestPath = 'game/data/game_manifest.json';
  let manifest = {
    startupScreen: 'game/data/screens/startup_screen.json',
    firstScene: 'game/data/scenes/ch00_q00_first_scene.json'
  };
  ...
  async function loadJson(path, fallback) {
    try {
      ...
      console.warn('Forever Bound JSON fallback', path, error);
  ...
  function renderStartup(data) {
    ...
    document.querySelector('.fb-studio-mark span').textContent = branding.studioName || 'CINAEDVS Studios';
    ...
    logo.alt = branding.fallbackTitle || 'Forever Bound';
    fallback.textContent = branding.fallbackTitle || 'Forever Bound';
  ...
  function visualFilter(visual = {}) {
    ...
    if (glow > 0) filters.push(`drop-shadow(0 0 ${Math.round(3 + glow * 0.24)}px rgba(195,0,255,.75))`);
    return filters.join(' ');
  }
  ...
  function controlsMarkup() {
    return '<div class="fb-controls"><div class="fb-dpad"><button class="fb-control-button fb-dpad-up" data-key="ArrowUp">U</button><button class="fb-control-button fb-dpad-down" data-key="ArrowDown">D</button><button class="fb-control-button fb-dpad-left" data-key="ArrowLeft">L</button><button class="fb-control-button fb-dpad-right" data-key="ArrowRight">R</button><div class="fb-dpad-center"></div></div><div class="fb-center-pad"><button class="fb-control-button">Select</button><button class="fb-control-button">Start</button></div><div class="fb-action-pad"><button class="fb-control-button fb-button-x">X</button><button class="fb-control-button fb-button-y">Y</button><button class="fb-control-button fb-button-b">B</button><button class="fb-control-button fb-button-a">A</button></div></div>';
  }
  ...
  function renderScene(data) {
    ...
    root.innerHTML = `<div class="fb-game-shell"><main class="fb-game-frame"><section class="fb-screen fb-play-screen"><div class="fb-scene-bg"></div>${objects}<div class="fb-vignette"></div><div class="fb-hud"><div class="fb-hud-panel fb-hearts">HP</div><div class="fb-hud-panel"><div class="fb-status-title">${escapeHtml(data.name || data.id || 'Forever Bound')}</div><div class="fb-status-line">${escapeHtml(data.calling || data.quest || '')}</div></div><div class="fb-hud-panel fb-active-item"><div class="fb-active-item-label">Active Item</div><div class="fb-active-item-name">Empty Hands</div></div></div><div class="fb-player"></div>${controlsMarkup()}</section></main></div>`;
    ...
    if (playerNode) {
      playerNode.style.backgroundImage = `url("${player.image || player.sprite || ''}")`;
      playerNode.style.width = `${Number(player.widthPercent ?? 13.5)}%`;
    }
    bindControls();
    updatePlayer();
  }
  ...
  function fallbackScene() {
    return { name: 'Fallback Scene', background: {}, player: { image: 'assets/characters/mel/mel_idle.png' }, travel: { groundY: 790 }, elements: [], objects: [] };
  }

  async function boot() {
    root.innerHTML = '<div class="fb-game-shell"><main class="fb-game-frame"><div class="fb-loading">Loading Forever Bound...</div></main></div>';
    manifest = await loadJson(manifestPath, manifest);
    const startup = await loadJson(manifest.startupScreen, { branding: { fallbackTitle: 'Forever Bound' }, buttons: [{ label: 'Start Game', action: 'startGame' }] });
  ...
  boot().catch((error) => {
    console.error(error);
    root.innerHTML = `<div class="fb-game-shell"><main class="fb-game-frame"><div class="fb-error"><strong>Forever Bound could not load.</strong><br>${escapeHtml(error.message)}</div></main></div>`;
  });
})();
