...
function objectList(): SceneObject[] {
...
function objectStoreKey(): "objects" | "entities" {
  return Array.isArray(scene.entities) && !Array.isArray(scene.objects) ? "entities" : "objects";
}

function setObjectList(objects: SceneObject[]): void {
...
function textInput(label: string, id: string, value: string): string {
  return `
...
function objectRow(object: SceneObject): string {
  return `
...
function bindEvents(): void {
  document.querySelector<HTMLInputElement>("#jsonImport")?.addEventListener("change", importJson);
  document.querySelector<HTMLInputElement>("#bgImport")?.addEventListener("change", importBackgroundPreview);
  document.querySelector<HTMLButtonElement>("#downloadJson")?.addEventListener("click", downloadJson);
  document.querySelector<HTMLButtonElement>("#copyJson")?.addEventListener("click", copyJson);
  document.querySelector<HTMLButtonElement>("#resetDraft")?.addEventListener("click", resetDraft);
  ...
  document.querySelector<HTMLButtonElement>("#selectPlayer")?.addEventListener("click", () => {
    selectedId = "playerStart";
    render();
  });

  document.querySelector<HTMLButtonElement>("#selectWalkable")?.addEventListener("click", () => {
    selectedId = "walkableArea";
    render();
  });

  document.querySelector<HTMLButtonElement>("#toggleGrid")?.addEventListener("click", () => {
    gridVisible = !gridVisible;
    render();
  });

  document.querySelector<HTMLButtonElement>("#addObject")?.addEventListener("click", addObject);

  document.querySelectorAll<HTMLButtonElement>("[data-select-object]").forEach((button) => {
  ...
  document.querySelectorAll<HTMLInputElement>("[data-object-label]").forEach((input) => {
    input.addEventListener("input", () => {
      updateObject(input.dataset.objectLabel, { label: input.value });
    });
  });

  document.querySelectorAll<HTMLInputElement>("[data-object-type]").forEach((input) => {
    input.addEventListener("input", () => {
...
function updateObject(id: string | undefined, patch: Partial<SceneObject>): void {
  if (!id) return;
...
function importJson(event: Event): void {
  ...
  reader.onload = () => {
    try {
      scene = JSON.parse(String(reader.result)) as SceneData;
      selectedId = "playerStart";
      saveDraft();
      render();
    } catch (error) {
...
function importBackgroundPreview(event: Event): void {
  const input = event.target as HTMLInputElement;
  const file = input.files?.[0];
