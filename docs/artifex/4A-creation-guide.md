...
Current verified implementation baseline: `Artifex Creation Guide V1.1.12` on `main` commit `3b5f94701d1400d64e30617fb0434ac6bcb30627`  
...
The current implementation evidence is `artifex/apps/creation-guide/index.html`, whose visible title and version badge identify **Artifex Creation Guide V1.1.12** at the verified baseline above.
...
Exact JSON shapes belong only in `docs/artifex/19a-project-starter-file-schemas.md`. Creation Guide's unique rule is that it starts this empty-project initialisation action; it does not populate later module-owned content.
...
After this specification is accepted and any still-live tasks are represented in `2A`, the older Creation Guide status/specification files are eligible for archive or targeted cleanup treatment rather than continuing as parallel current authorities.

## Remaining Work

All current and future Creation Guide tasks are owned by `docs/artifex/2A-global-to-do.md`. This specification must not accumulate task checklists.