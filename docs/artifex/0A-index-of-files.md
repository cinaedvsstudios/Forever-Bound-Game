...
Latest baseline checked for this update: Effect Editor `INDEX2-CLEAN-0.2.6` on current `main`, audited 3 June 2026 from the accepted `index2.html` route and its active module chain  
...
| `docs/artifex/9A-effect-editor.md` | Effect Editor module specification document. | Added during Effect Editor extraction pass; verified against accepted `INDEX2-CLEAN-0.2.6` implementation. |
...
| 8 | Effect Editor | Reusable FX ownership and accepted Index2 baseline/route. | Extracted into `docs/artifex/9A-effect-editor.md`; implementation follow-up remains in `2A`. |
| 9 | Asset Library | Promotion and final registration of supplied/generated assets. | Next module audit; decide whether existing asset workflow content forms this spec. |
...
| `docs/artifex/13-effects-library.md` | Valid Effect Editor / FX Archetype / FX Instance / possible Plate FX boundary content moves into `9A`; older prototype and noncanonical path claims are not current Index2 authority. | Retain as source evidence now; archive or reduce to a bounded historical/concept reference after `9A` is accepted. |
| `artifex/apps/effect-editor/docs/todo.md` | Still-live Effect Editor work moves into `2A`; its emitter-width marker item is superseded because that guide is present in accepted Index2. | Retain as source evidence now; archive or replace with a minimal local pointer after `9A` acceptance. |
| `artifex/apps/effect-editor/docs/current-vs-stable-ui-inventory.md`, `compare-versions.md`, `UI_STRUCTURE_AUDIT.md`, `PHASE_1_SAFE_FILE_SPLIT.md` and `PHASE_2_UI_CLEANUP.md` | Earlier default-route recovery/design/refactor evidence; current capability must be verified from Index2 owners. | Archive/history after `9A` acceptance; not active specification or backlog authority. |
| `artifex/apps/effect-editor/docs/PHASE_8_LIBRARY_IMPORT_NOTES.md`, `FX_UI_RESTRUCTURE_NOTES.md`, `FX_QUICK_EDIT_BRUSH_AND_REFERENCE_NOTES.md`, `FX_QUICK_EDIT_FIX_NOTES.md`, `FX_PHASE9I_ORIGIN_BRUSH_LIBRARY_NOTES.md`, `FX_PHASE9J_RESOLUTION_NOISE_NOTES.md`, `v3-roadmap-open-items.md` and `src/README.md` | Old route/phased delivery or refactor evidence; features were recorded only where independently verified in accepted Index2. | Archive/history after extraction; do not use as current Index2 authority. |
| `artifex/apps/effect-editor/docs/future-features.md` | Deferred Brush Sequence Animation idea only; it is not current functionality or required current work. | Retain only as deferred idea material or archive after deliberate future classification. |
| `docs/effect-editor/index2-integration-work-started.md` | Earlier Index2 0.2.2 checkpoint; superseded for current status by accepted Index2 0.2.6. | Historical checkpoint/archive candidate after `9A` acceptance. |
| `artifex/shared/todo-guide/audits/2026-06-01-effect-editor-route-decision-audit.md` and relevant baseline refresh evidence | Route decision and accepted Index2 evidence move into `9A`; original `0.2.3` missing-parity statements are superseded by verified `0.2.6`. | Retain as evidence now; archive or supersede after `9A` acceptance. |
| `artifex/apps/effect-editor/debug/atmosphere-volume/README.md` and merged PR #49 | Isolated visual-prototype boundary is captured in `9A`; it is not live runtime, saved FX Archetype support or Scene Editor placement. | Retain as prototype evidence until a separate production integration decision is made. |
| `docs/archive/effect-editor-v3-promotion/**` and archived old Effect Editor handoff records | Already historical default-route/promotion material. | Archive evidence only; not current authority. |
...
| `docs/artifex/19a-project-starter-file-schemas.md` | Remains the subordinate exact schema technical reference linked from `1A`, `4A`, `5A`, `7A`, `8A` and `9A`. | Retain active. |
...
| `docs/artifex/24-stabilisation-cleanup-and-ui-resumption-plan.md` | Permanent no-patch/one-scope/verified-archive/process rules move into `1A`; remaining open work moves into `docs/artifex/2A-global-to-do.md`; its older Effect Editor `0.2.3` parity position is superseded by `9A`. | Archive after extraction. |
