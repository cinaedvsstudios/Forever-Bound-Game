...
Current verified implementation baseline: `Artifex Archetype Object Creator V1.36` on current `main`, audited 3 June 2026  
Accepted implementation evidence: merged PR #38 / merge commit `ef4f37ebe5850c6367db59e57c01e2bb89949384`  
...
This safety contract is implemented in V1.36 code but remains to be proven through the post-merge functional validation listed in `2A`.
...
`artifex/apps/archetype-object-creator/docs/todo.md` currently duplicates active task ownership; all still-live Object Creator work is represented in `2A` so this app-specific task file can later become archive evidence rather than a competing backlog.
...
All current and future Archetype Object Creator tasks are owned by `docs/artifex/2A-global-to-do.md`. This specification must not accumulate task checklists.