# Colour and Display Rules Reference

Status: Active visual/style reference during documentation consolidation  
Reference role: exact Artifex colour, display and UI styling rules  
Source evidence: `docs/artifex/18-color-and-display-rules.md`  
Related document: `docs/artifex/01A-project-file-contracts.md`

## Purpose

This reference preserves exact starter colour and display rules plus current Artifex UI branding rules used by editor modules.

The universal contract may summarise branding requirements, but this file keeps the detailed palette and UI treatment so old `18-color-and-display-rules.md` can be archived safely.
...
- Module-specific accent colours identify the current tool family.
- Module accents must not become a user-facing theme picker unless explicitly required.
...
The earlier phrase “Project / Library tools: gold-green accent” should not be used because it is ambiguous.
...
- Use an emoji or symbol when the action is simple: pause/play, snapshot, clear, save, delete, underlay, performance mode, collapse, expand, move up, move down, duplicate, show/hide, reset or zoom target.
...
Keep that image as a visual palette reference or archive it with older docs only after this reference is accepted.
