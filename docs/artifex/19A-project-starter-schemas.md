...
Date: 2026-05-31  
Status: Active subordinate schema reference for the Artifex project-file contract

## Purpose

`docs/artifex/01A-project-file-contracts.md` defines ownership and folder paths. This companion document defines the minimum JSON shape for the files Creation Guide initially creates and Project Editor subsequently opens, validates and writes.
...
5. IDs use the prefixes in `docs/artifex/01A-project-file-contracts.md`.
...
| `intake/dialogue-sfx/` | Drop source voice/dialogue sound effects and temporary dialogue audio captures. | Files here are staging files only. After approval they must be copied/promoted into final `assets/audio/` folders and registered in Asset Library or Sound Library as appropriate. |
| `intake/icons-ui/` | Drop source icons, UI marks, buttons, prompts and interface images. | Files here are staging files only. After approval they must be copied/promoted into final `assets/` folders and registered in Asset Library. |
| `intake/music/` | Drop source music cues, draft loops and temporary soundtrack material. | Files here are staging files only. After approval they must be copied/promoted into final `assets/audio/music/` paths and registered in Asset Library or Sound Library as appropriate. |
| `intake/objects/` | Drop source props, pickups, doors, interactable objects, object sheets and object references before Object Creator finalisation. | Files here are staging files only. Ready gameplay objects must be promoted into final assets and canonical object archetype records. |
| `artifex/templates/starter-asset-placeholders/` | Stores rough placeholder images/icons/audio for newly started Artifex projects. | Placeholder README files are not schema authority; their useful folder rules are captured here and their asset lists belong to Asset Library/Sound Library specs when still useful. |
